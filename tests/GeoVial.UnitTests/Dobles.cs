...
internal sealed class FakeRelevamientoRepository : IRelevamientoRepository
{
    ...
    public Task<IReadOnlyList<Relevamiento>> ListarPorAreaAsync(Guid areaId, CancellationToken ct = default) =>
        Task.FromResult<IReadOnlyList<Relevamiento>>(_datos.Values.Where(r => r.AreaId == areaId).ToList());
